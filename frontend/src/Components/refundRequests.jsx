...
const RefundRequests = () => {
    ...
    useEffect(()=>{
        const getRequests = async () =>{
            setReady(false);
            const res = await courseService.getRefundRequests();
            setRequests(res);
            setReady(true);
        }
        getRequests();
    ...
    const accept = (course) =>{
        swal({
            title: `Do you really want to Refund This Course` ,
            icon: "warning",
            buttons: true,
            dangerMode: true,
          })
          .then(async (willDelete) => {
            if (willDelete) {
              swal("The course has been refunded.", {
                icon: "success",
              });
              const res = await courseService.AdminRefundCourse(course);
              setReload(!reload);
            }
          });
    };
                    ...
                    return <div key={request._id} className="card bg-light m-2">
                        <div className="row">
                            ...
                            <div className="col-sm-8">
                                <p>{request.studentName} requested to refund {request.title} Course</p>
                            </div>
